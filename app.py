...
for cat in cats:
    ...
    if len(subset) >= 2:  # at least two samples to compute r
...
total_actual  = ve['reach'].sum()
total_pred    = ve['predicted_reach'].sum()
deviation_pct = (abs(total_actual - total_pred) / total_pred * 100) if total_pred else 0

col1.metric("Total Actual Reach",    fmt(total_actual))
...
with st.expander("🧠 Why is the deviation high?"):
    st.markdown("- Outlier/collaboration posts skew totals")
...
if not ve.empty:
    ...
    st.write("No Viral or Excellent reels in this date range.")
...
if api_key and 'caption' in df.columns:
    ...
    st.info("Add OPENAI_API_KEY to your Streamlit secrets to enable Content Intelligence.")
...
if go:
    ...
    if perf in promotion:
        ...
        if rcat.notna().any():
            ...
            st.markdown(
                f"🔜 To move **{perf}** → **{target}**, "
                f"focus on **{best_m.capitalize()}** (r={best_v:.2f})."
            )
        else:
